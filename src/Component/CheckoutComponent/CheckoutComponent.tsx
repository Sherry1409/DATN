...
const CheckoutComponent: React.FC = () => {
  ...
  const paymentStatus = useSelector((state: RootState) => state.order.payment_status)
  const paymentMessage = useSelector((state: RootState) => state.order.payment_message)
  ...
  useEffect(() => {
    const appliedDiscount = voucherId
      ? Number(vouchers.find((voucher) => voucher.id === voucherId)?.discount_value) || 0
      : 0
                  ...
                  <div className='box-total-checkout'>
                    ...
                    <div className='box-footer-checkout'>
                      <div className='form-group'>
                        {isVoucher?.length > 0 && (
                          <div className='mb-3'>
                            <select className='chon-vou' onChange={handleVoucherChange} value={voucherId || ''}>
                              <option value='' disabled>
                                Chọn mã giảm giá
                              </option>
                              {isVoucher.map((voucher) => (
                                <option key={voucher.id} value={voucher.id}>
                                  {voucher.code} - Giảm {formatCurrency(voucher.discount_value)}
                                </option>
                              ))}
                            </select>
                          </div>
                        ...
                        {isVoucher?.length > 0 && (
                          <div className='item-checkout justify-content-between'>
                            ...
                            <span className='font-md-bold'>{discount > 0 ? formatCurrency(discount) : '0'}</span>
                          </div>
                        )}
                        <div className='item-checkout justify-content-between'>
